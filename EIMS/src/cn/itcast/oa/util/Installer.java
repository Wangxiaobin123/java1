...
public class Installer {
	...
	public void installer(){
		...
		session.save(menu1);
		session.save(menu2);
		session.save(new Privilege("置顶", "/topic_setTop", menu2));

		menu = new Privilege("资料上传", null, null);
		menu1 = new Privilege("上传资料", "/forumManage_list2", menu);
		session.save(menu);
		session.save(menu1);
		
		menu = new Privilege("查看列表", null, null);
		menu1 = new Privilege("岗位查看", "/role_list", menu);
		menu2 = new Privilege("部门查看", "/department_list", menu);
		menu3 = new Privilege("员工信息", "/user_list", menu);
		...
		session.save(new Privilege("修改个人信息", "/user_edit2", menu3));
